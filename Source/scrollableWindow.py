...
class ScrollableWindow(Sprite):
    ...
    def isClicked(self, xy):        
        ...
        if self.mode == "groceryListViewer":
            return None
        ...
        for sprite in self.allElements:
            if sprite.getRect().collidepoint(xy):
                ...
                if buttonClicked == None:
                    ...
                    # this information
                    if buttonClicked.getIdentity().find("@") == -1:
                        return parseID
                        ...
                        #print "WITHIN SCROLLABLE WINDOW:"
                        #print "    ", command[0], "    ", recipeID
                        ...
                        if command[0] == "expand":
                            ...
                            return command[0], recipeID
                        elif command[0] == "delete":                            
                            sprite.toggleButton("deleteButton")
                            self.redrawAll()
                            return command[0], recipeID
    ...
    def writeToDesktop(self):
        if self.mode == "groceryListViewer":
            allIngredientList = ""
            for element in self.allElements:
                if isinstance(element, Row):
                    allIngredientList += element.getName() + "\n"
